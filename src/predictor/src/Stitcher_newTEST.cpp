...
Mat Kp = (Mat_<double>(3, 3) << 2120.49,	0,	981.1,
    0,	2119.06,	535.16,
    0,	0,	1);
...
double homCalc(double pitch0, double pitch1, double yaw0, double yaw1)
{    
...
void QuatCallback(const geometry_msgs::QuaternionStamped::ConstPtr& msg, int I) {
    //cout << I << endl;
...
void imageCb_omni(const sensor_msgs::ImageConstPtr& msg) {
	...
	// Undistort image
	fisheye::undistortImage(orig, orig_show_omni, Ko, D, Ko, Size(orig.cols, orig.rows));

	// Flip image
	//flip(orig, orig_show_omni, -1);
	
	//imshow("image_omni", orig_show_omni);
    	//cv::waitKey(1);
}
...
void imageCb_ptz(const sensor_msgs::ImageConstPtr& msg) {
    ...
    // To show image
    Mat orig_show;
    resize(src_ptz, orig_show_PTZ, src_ptz.size(), 0);
...
int main(int argc, char** argv){
    ...
    ros::Subscriber sub = node.subscribe<geometry_msgs::QuaternionStamped>("/unnamed/control/camera_orientation", 1, boost::bind(&QuatCallback, _1, INT));
